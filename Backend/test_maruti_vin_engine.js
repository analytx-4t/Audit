...
async function testMarutiVinEngine() {
  const excelPath = `C:\\Users\\hp\\Downloads\\1_Result\\vehicles.xlsx`;
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(excelPath);
  const sheet = workbook.getWorksheet(1);

  console.log("Testing Maruti Suzuki VIN Engine Corrector on all 11 Ground Truth rows...\n");
  ...
  console.log(`Summary: Successfully matched ${matchCount} out of ${totalCount} ground truth rows! (${Math.round((matchCount/totalCount)*100)}% accuracy)`);
}

testMarutiVinEngine().catch(console.error);
